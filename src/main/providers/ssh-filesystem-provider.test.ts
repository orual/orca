...
describe('SshFilesystemProvider', () => {
  ...
  describe('writePrivateFile', () => {
    it('sends fs.writePrivateFile request', async () => {
      mux.request.mockResolvedValue('/home/user/.orca/jj/setup/abcd/setup-runner.sh')
      await expect(provider.writePrivateFile('abcd', 'sh', '#!/bin/sh\n')).resolves.toBe(
        '/home/user/.orca/jj/setup/abcd/setup-runner.sh'
      )
      expect(mux.request).toHaveBeenCalledWith('fs.writePrivateFile', {
        workspaceKey: 'abcd',
        extension: 'sh',
        content: '#!/bin/sh\n'
      })
    ...
    it('fails closed with reconnect guidance when the relay lacks private runner storage', async () => {
      mux.request.mockRejectedValue(Object.assign(new Error('Method not found'), { code: -32601 }))
